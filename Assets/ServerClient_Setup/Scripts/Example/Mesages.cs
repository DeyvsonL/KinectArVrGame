...
public class Mesages : NetworkBehaviour
{
    ...
    public GameObject leftArmGameObject;
    //Setting up the SyncVar.
    [SyncVar(hook = "OnVarSynced")]
    //this needs to be above the variable you want to sync. (you can name the "Hook" everything you want, but be aware that this is the name of a function that is called on the clients when the SyncVar changes on the server)
    ...
    public void OnVarSynced(string syncedVar)
        //this is the "Hook" function that is called on clients when the Syncvar changes on the server.
    {
    ...
    void Update()
    {
        ...
        {
            leftArm.transform.position = new Vector3(leftArmGameObject.transform.position.x, leftArmGameObject.transform.position.y + 1, leftArmGameObject.transform.position.z);
        }
#endif
            if (isServer)
